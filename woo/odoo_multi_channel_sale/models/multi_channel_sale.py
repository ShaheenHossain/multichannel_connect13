...
from woocommerce_v11.woo.odoo_multi_channel_sale.tools import chunks,  DomainVals, ReverseDict
...
class MultiChannelSale(models.Model):
    ...
    channel_company_id = fields.Many2one('res.company', string='Instance Company')
    channel_salesperson = fields.Many2one('res.users', string='Instance SalesPerson')
    ...
    def _create_sync(self, vals):
        if self.debug=='enable':
            ...
            return self.env['channel.synchronization'].sudo().create(nvals)
    ...
    def create_order_mapping(self, erp_id, store_id,store_source=None):
        ...
        channel_vals = self.get_channel_vals()
        vals.update(channel_vals)
        return self.env['channel.order.mappings'].sudo().create(vals)
