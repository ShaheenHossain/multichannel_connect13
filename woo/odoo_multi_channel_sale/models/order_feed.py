...
from custom.woo.odoo_multi_channel_sale.tools import parse_float, extract_list as EL
import logging
...
class OrderFeed(models.Model):
            ...
            for line_id in self.env['order.line.feed'].browse(line_ids):
                ...
                if product_id:
                    ...
                    line = dict(
                        ...
                        company_id=channel_id.channel_company_id.id,
                    )
    ...
    def import_order(self,channel_id):
        _logger.info("============================== We are in import Order ==========================================")
        message = ""
        update_id=None
        create_id=None
        self.ensure_one()
        ...
        vals['company_id'] = channel_id.channel_company_id.id
        vals['user_id'] = channel_id.channel_salesperson.id
            ...
            if  state =='done' :
                try:
                    ...
                    if match.order_name.state=='draft':
                        match.order_name.sudo().write(dict(order_line=[(5,0)]))
                        match.order_name.sudo().write(vals)
    ...
    @api.multi
    def import_items(self):
        _logger.info("=========================== We are in import Items ==================================================================")
        update_ids=[]
        create_ids=[]
        message=''
        for record in self:
