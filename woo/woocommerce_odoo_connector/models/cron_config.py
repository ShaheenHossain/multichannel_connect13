	...
	def write(self, vals):
		...
		for channel_id in self:
			if channel_id.channel == 'woocommerce':
				if ( ('woocommerce_is_import' in vals) or ('woocommerce_intervals' in vals) 
					or ('woocommerce_interval_type' in vals) or ('woocommerce_feed_cron' in vals) 
					or ('woocommerce_is_export' in vals) ):
