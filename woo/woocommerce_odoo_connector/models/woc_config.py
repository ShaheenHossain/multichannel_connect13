...
class MultiChannelSale(models.Model):
	...
	def get_woocommerce_connection(self):
		_logger.info("==================== Now In Woocomerce Connection to check connection ===================")
		woocommerce = API(
			url 			=	self.url,
			consumer_key	=	self.consumer_key,
			consumer_secret	=	self.secret_key,
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@---Import--@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@-
	...
	def import_woocommerce_orders_cron(self):
		...
		for channel in all_channel:
			if channel.woocommerce_is_import:
				try:
					_logger.info("==================== Now In Import Woocomerce Cron ===================")
	...
	@api.model
	def import_woocommerce_orders_cron_instance_two(self):
		all_channel = self.env['multi.channel.sale'].search([('id', '=', 2)])
		if all_channel:
			for channel in all_channel:
				if channel.woocommerce_is_import:
					try:
						_logger.info("=========== Successfully Enter in cron Py ==========================")
						channel.import_woocommerce_orders()
					except Exception as e:
						_logger.info("=====Order Evaluate Failed (WooCommerce)====(%r)====(%r)", channel, e)
						continue
	...
	@api.model
	def update_woocommerce_orders_cron(self):
		all_channel = self.env['multi.channel.sale'].search([('channel', '=', 'woocommerce')])
		for channel in all_channel:
			if channel.woocommerce_is_import:
				try:
					...
					_logger.info("=====update Order Evaluate Failed (WooCommerce)====(%r)====(%r)", channel, e)
					continue
		return True
