...
class MultiChannelSale(models.Model):
	...
	def create_or_get_woocommerce_voucher(self, vouchers):
		...
		for voucher in vouchers:
			voucher_line = {
							'line_name'		  		: "Voucher",
							'line_price_unit' 		: voucher['discount'],
	...
	def get_woocommerce_order_line(self, data):
		...
		for line in data:
			...
			if not product_template_id:
				product_template_id = self.env['channel.product.mappings'].search([('store_product_id','=',line['product_id']),('channel_id.id','=',self.id)], limit=1)
			order_line_dict = {
					'line_name'				:line['name'],
			...
			_logger.info("====line order_line_dict %r ",order_line_dict)
	...
	def import_woocommerce_orders(self):
		...
		try:
			...
			_logger.info("============ ====Now check feed Import process===================================")
