...
class MultiChannelSale(models.Model):
    ...
    def woc_check_and_create_tax(self,woocommerce):
        _logger.info("==================== Now In Woocomerce Tax Def ===================")
        if woocommerce:
            i = 1
            while(i):
                try:
                ...
                if 'message' in taxes:
                    ...
                    if taxes:
                        ...
                        for tax in taxes:
                            check = tax_map_obj.sudo().search([('store_id','=',int(tax['id'])),('channel_id.id','=',self.id)])
                            tax_srch = tax_obj.sudo().search([('name', '=',tax['name'])])
                            ...
                            if not check:
                                ...
                                _logger.info("====>>>>>>>taxes %r",(check,check.tax_name,amount,tax_rate,name, tax['name'] ))
